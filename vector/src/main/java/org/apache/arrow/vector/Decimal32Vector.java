...
import org.apache.arrow.vector.complex.impl.Decimal32ReaderImpl;
...
/**
 * Decimal32Vector implements a fixed width vector (4 bytes) of decimal values which could be null.
 * A validity buffer (bit vector) is maintained to track which elements in the vector are null.
 */
public final class Decimal32Vector extends BaseFixedWidthVector
  ...
  private static void checkByteLength(int length) {
    if (length < 1 || length > TYPE_WIDTH) {
      throw new IllegalArgumentException(
          "Invalid decimal value length. Valid length in [1 - " + TYPE_WIDTH + "], got " + length);
    }
  }

  /**
   * Instantiate a Decimal32Vector. This doesn't allocate any memory for the data in vector.
   *
   * @param name name of the vector
   * @param allocator allocator for memory management.
   */
  public Decimal32Vector(String name, BufferAllocator allocator, int precision, int scale) {
    this(
        name,
   ...
   * @param allocator allocator for memory management.
   */
  public Decimal32Vector(String name, FieldType fieldType, BufferAllocator allocator) {
   ...
   * @param allocator allocator for memory management.
   */
  public Decimal32Vector(Field field, BufferAllocator allocator) {
  ...
  protected FieldReader getReaderImpl() {
    return new Decimal32ReaderImpl(Decimal32Vector.this);
  }
  ...
  *----------------------------------------------------------------*/
   ...
   * @param index position of element
   */
  public void get(int index, NullableDecimal32Holder holder) {
    if (isSet(index) == 0) {
      holder.isSet = 0;
      return;
    }
  ...
  *----------------------------------------------------------------*/
  ...
  public void setBigEndian(int index, byte[] value) {
    ...
    if (length > TYPE_WIDTH) {
      throw new IllegalArgumentException(
          "Invalid decimal value length. Valid length in [0 - " + TYPE_WIDTH + "], got " + length);
    }
    BitVectorHelper.setBit(validityBuffer, index);
    ...
    if (length == 0) {
      MemoryUtil.setMemory(outAddress, Decimal32Vector.TYPE_WIDTH, (byte) 0);
      return;
    }
    if (LITTLE_ENDIAN) {
      ...
      if (length < TYPE_WIDTH) {
        ...
        MemoryUtil.setMemory(outAddress + length, Decimal32Vector.TYPE_WIDTH - length, pad);
      }
    } else {
      // copy data from value to outAddress
      MemoryUtil.copyToMemory(value, 0, outAddress + Decimal32Vector.TYPE_WIDTH - length, length);
      if (length < TYPE_WIDTH) {
        // sign extend
        final byte pad = (byte) (value[0] < 0 ? 0xFF : 0x00);
        MemoryUtil.setMemory(outAddress, Decimal32Vector.TYPE_WIDTH - length, pad);
      }
    }
  }
  ...
  /**
   * Sets the element at given index using the buffer whose size maybe <= 4 bytes.
   *
  ...
  public void setSafe(int index, long start, ArrowBuf buffer, int length) {
    ...
    if (LITTLE_ENDIAN) {
      ...
      MemoryUtil.copyMemory(inAddress, outAddress + Decimal32Vector.TYPE_WIDTH - length, length);
      // sign extend
      if (length < TYPE_WIDTH) {
  ...
  public void set(int index, long value) {
    // write the value first: it range-checks and may throw, and the slot must
    // not be marked valid if the write does not happen.
   ...
   */
  public void set(int index, NullableDecimal32Holder holder) throws IllegalArgumentException {
    if (holder.isSet < 0) {
      throw new IllegalArgumentException();
   ...
   */
  public void set(int index, Decimal32Holder holder) {
    BitVectorHelper.setBit(validityBuffer, index);
    valueBuffer.setBytes((long) index * TYPE_WIDTH, holder.buffer, holder.start, TYPE_WIDTH);
  }
  ...
  /**
   * Same as {@link #set(int, NullableDecimal32Holder)} except that it handles the case when index
   * is greater than or equal to existing value capacity {@link #getValueCapacity()}.
   *
   * @param index position of element
   * @param holder nullable data holder for value of element
   */
  public void setSafe(int index, NullableDecimal32Holder holder) throws IllegalArgumentException {
    handleSafe(index);
    set(index, holder);
  }

  /**
   * Same as {@link #set(int, Decimal32Holder)} except that it handles the case when index is
   * greater than or equal to existing value capacity {@link #getValueCapacity()}.
   *
   * @param index position of element
   * @param holder data holder for value of element
   */
  public void setSafe(int index, Decimal32Holder holder) {
    handleSafe(index);
    set(index, holder);
  }
    ...
    for (int i = 0; i < getValueCount(); ++i) {
      ...
      if (value != null) {
        ValidateUtil.validateOrThrow(
            ...
            "Invalid value for Decimal32Vector at position "
                + i
  ...
  *----------------------------------------------------------------*/
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((Decimal32Vector) to);
  }
  ...
  private class TransferImpl implements TransferPair {
    ...
    public TransferImpl(String ref, BufferAllocator allocator) {
      to =
          (Decimal32Vector.this.field != null && Decimal32Vector.this.field.getFieldType() != null)
              ? new Decimal32Vector(ref, Decimal32Vector.this.field.getFieldType(), allocator)
              : new Decimal32Vector(
                  ref, allocator, Decimal32Vector.this.precision, Decimal32Vector.this.scale);
    }

    public TransferImpl(Field field, BufferAllocator allocator) {
      to = new Decimal32Vector(field, allocator);
    }

    public TransferImpl(Decimal32Vector to) {
      this.to = to;
    }

    @Override
    public Decimal32Vector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, Decimal32Vector.this);
    }
  }
}
