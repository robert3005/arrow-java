...
import org.apache.arrow.vector.complex.impl.Decimal64ReaderImpl;
...
/**
 * Decimal64Vector implements a fixed width vector (8 bytes) of decimal values which could be null.
 * A validity buffer (bit vector) is maintained to track which elements in the vector are null.
 */
public final class Decimal64Vector extends BaseFixedWidthVector
  ...
  /**
   * Instantiate a Decimal64Vector. This doesn't allocate any memory for the data in vector.
   *
   * @param name name of the vector
   * @param allocator allocator for memory management.
   */
  public Decimal64Vector(String name, BufferAllocator allocator, int precision, int scale) {
    this(
        name,
   ...
   * @param allocator allocator for memory management.
   */
  public Decimal64Vector(String name, FieldType fieldType, BufferAllocator allocator) {
   ...
   * @param allocator allocator for memory management.
   */
  public Decimal64Vector(Field field, BufferAllocator allocator) {
  ...
  protected FieldReader getReaderImpl() {
    return new Decimal64ReaderImpl(Decimal64Vector.this);
  }
  ...
  *----------------------------------------------------------------*/
   ...
   * @param index position of element
   */
  public void get(int index, NullableDecimal64Holder holder) {
    if (isSet(index) == 0) {
      holder.isSet = 0;
      return;
    }
  ...
  *----------------------------------------------------------------*/
  ...
  public void setBigEndian(int index, byte[] value) {
    ...
    if (length == 0) {
      MemoryUtil.setMemory(outAddress, Decimal64Vector.TYPE_WIDTH, (byte) 0);
      return;
    }
    if (LITTLE_ENDIAN) {
      ...
      if (length < TYPE_WIDTH) {
        ...
        MemoryUtil.setMemory(outAddress + length, Decimal64Vector.TYPE_WIDTH - length, pad);
      }
    } else {
      // copy data from value to outAddress
      MemoryUtil.copyToMemory(value, 0, outAddress + Decimal64Vector.TYPE_WIDTH - length, length);
      if (length < TYPE_WIDTH) {
        // sign extend
        final byte pad = (byte) (value[0] < 0 ? 0xFF : 0x00);
        MemoryUtil.setMemory(outAddress, Decimal64Vector.TYPE_WIDTH - length, pad);
      }
    }
  }
  ...
  /**
   * Sets the element at given index using the buffer whose size maybe <= 8 bytes.
   *
  ...
  public void setSafe(int index, long start, ArrowBuf buffer, int length) {
    ...
    if (LITTLE_ENDIAN) {
      ...
      MemoryUtil.copyMemory(inAddress, outAddress + Decimal64Vector.TYPE_WIDTH - length, length);
      // sign extend
      if (length < TYPE_WIDTH) {
   ...
   */
  public void set(int index, NullableDecimal64Holder holder) throws IllegalArgumentException {
    if (holder.isSet < 0) {
      throw new IllegalArgumentException();
   ...
   */
  public void set(int index, Decimal64Holder holder) {
    BitVectorHelper.setBit(validityBuffer, index);
    valueBuffer.setBytes((long) index * TYPE_WIDTH, holder.buffer, holder.start, TYPE_WIDTH);
  }
  ...
  /**
   * Same as {@link #set(int, NullableDecimal64Holder)} except that it handles the case when index
   * is greater than or equal to existing value capacity {@link #getValueCapacity()}.
   *
   * @param index position of element
   * @param holder nullable data holder for value of element
   */
  public void setSafe(int index, NullableDecimal64Holder holder) throws IllegalArgumentException {
    handleSafe(index);
    set(index, holder);
  }

  /**
   * Same as {@link #set(int, Decimal64Holder)} except that it handles the case when index is
   * greater than or equal to existing value capacity {@link #getValueCapacity()}.
   *
   * @param index position of element
   * @param holder data holder for value of element
   */
  public void setSafe(int index, Decimal64Holder holder) {
    handleSafe(index);
    set(index, holder);
  }
    ...
    for (int i = 0; i < getValueCount(); ++i) {
      ...
      if (value != null) {
        ValidateUtil.validateOrThrow(
            ...
            "Invalid value for Decimal64Vector at position "
                + i
  ...
  *----------------------------------------------------------------*/
  ...
  public TransferPair makeTransferPair(ValueVector to) {
    return new TransferImpl((Decimal64Vector) to);
  }
  ...
  private class TransferImpl implements TransferPair {
    ...
    public TransferImpl(String ref, BufferAllocator allocator) {
      to =
          (Decimal64Vector.this.field != null && Decimal64Vector.this.field.getFieldType() != null)
              ? new Decimal64Vector(ref, Decimal64Vector.this.field.getFieldType(), allocator)
              : new Decimal64Vector(
                  ref, allocator, Decimal64Vector.this.precision, Decimal64Vector.this.scale);
    }

    public TransferImpl(Field field, BufferAllocator allocator) {
      to = new Decimal64Vector(field, allocator);
    }

    public TransferImpl(Decimal64Vector to) {
      this.to = to;
    }

    @Override
    public Decimal64Vector getTo() {
      return to;
    }
    ...
    public void copyValueSafe(int fromIndex, int toIndex) {
      to.copyFromSafe(fromIndex, toIndex, Decimal64Vector.this);
    }
  }
}
